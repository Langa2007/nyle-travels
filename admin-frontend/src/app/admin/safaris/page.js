  ...
  const fetchData = async () => {
    try {
      ...
      const [toursRes, statsRes] = await Promise.all([
        toursAPI.getAll({ type: 'safari', search: searchTerm, difficulty: filterDifficulty, limit: 100 }),
        toursAPI.getStats()
      ]);
                  ...
                  <tr key={tour.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-8 py-6 border-b border-gray-100 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <a href={process.env.NEXT_PUBLIC_FRONTEND_URL ? `${process.env.NEXT_PUBLIC_FRONTEND_URL}/safaris/${tour.slug}` : `http://localhost:3000/safaris/${tour.slug}`} target="_blank" rel="noreferrer" className="p-2 rounded-xl bg-gray-50 text-gray-400 hover:text-primary-500 transition-colors">
                          <FiEye size={18} />
                        </a>
                        <Link href={`/admin/safaris/${tour.id}`} className="p-2 rounded-xl bg-gray-50 text-gray-400 hover:text-primary-500 transition-colors">
                          <FiEdit2 size={18} />
                        </Link>
                        <button 
