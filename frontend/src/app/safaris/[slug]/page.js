  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <Button href="/safaris" variant="primary">Return to Catalog</Button>
      </div>
    );
  }
        ...
        {similarTours?.length > 0 && (
          <section className="mt-40 border-t border-gray-100 pt-24">
            <div className="flex items-center justify-between mb-16 px-4">
              ...
              <Button href="/safaris" variant="outline" className="!rounded-full px-8">Explore Catalog</Button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {similarTours.map(simTour => (
                <TourCard key={simTour.id} tour={simTour} basePath="/safaris" />
              ))}
            </div>
          </section>
        )}
